...
def readcsv(datei):
    ...
    return(X,Y,Z);
"""
#旧代码--baseline training data generator
...
def make_train_data(n, nr, diff=(0x0040,0)):
  ...
  Y = Y & 1
  keys = rng.integers(0, 65535, size=(4, n))
  plain0l = rng.integers(0, 65535, size=n)
  plain0r = rng.integers(0, 65535, size=n)
  plain1l = plain0l ^ diff[0]; plain1r = plain0r ^ diff[1]
  num_rand_samples = np.sum(Y==0)
  # plain1l[Y==0] = np.frombuffer(urandom(2*num_rand_samples),dtype=np.uint16);
  # plain1r[Y==0] = np.frombuffer(urandom(2*num_rand_samples),dtype=np.uint16);
  plain1l[Y==0] = rng.integers(0, 65535, size=num_rand_samples)
  plain1r[Y == 0] = rng.integers(0, 65535, size=num_rand_samples)
  ks = expand_key(keys, nr)
  ctdata0l, ctdata0r = encrypt((plain0l, plain0r), ks)
  ctdata1l, ctdata1r = encrypt((plain1l, plain1r), ks)
  X = convert_to_binary([ctdata0l,ctdata0r, ctdata1l, ctdata1r])
